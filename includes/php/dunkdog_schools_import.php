<?php
 ...
 class Dunkdog_Duplicate_Schools_Import{
	...
	function __construct(){
		...
		add_action('admin_menu', array(&$this, 'dunkdog_duplicate_school_menu'));
	}

	function dunkdog_duplicate_school_menu(){krumo("poo");
		require_once ABSPATH . '/wp-admin/admin.php';
		add_submenu_page ('edit.php?post_type=dd-team-ranking', 'Duplicate Schools Import', 'Duplicate Schools Import', 0, 'duplicate-school-import', array(&$this, 'dunkdog_duplicate_import_excel'));
	}
...
// import functions
	function dunkdog_duplicate_import_excel(){
		if ('POST' == $_SERVER['REQUEST_METHOD']) {
            $this->loadExcel();
        }
		?>			
    ...
    function loadExcel(){
		...
		if($excel->success()){
			...
			for($i=0; $i<count($data); $i++){
			 	$id = $this->fixDuplicateSchool($data[$i]);
    ...
    function fixDuplicateSchool($school){
    	$bad = $this->getID($school[0]);
    	$good = $this->getID($school[1]);
 ...
 $ddsi= new Dunkdog_Duplicate_Schools_Import();
